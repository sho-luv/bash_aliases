...
# my personal aliases
	...
	# grep nmsp nse scripts
	  alias nse='ls -l /usr/share/nmap/scripts | grep -i '
	...
	# metasploit aliases:
	  ...
	  alias ms17-010='msfconsole -qx "use exploit/windows/smb/ms17_010_eternalblue;"'
	  alias ms08-067='msfconsole -qx "use exploit/windows/smb/ms08_067_netapi;"'
	  alias ms08-067_psexec='msfconsole -qx "use exploit/windows/smb/ms08_067_netapi;"'

	  alias john-lm-all-dics='find /root/dic/ -name "*" | xargs -t -I file john lm -format:lm -w:file -fork:8'
	  alias john-nt-all-dics='find /root/dic/ -name "*" | xargs -t -I file john nt -format:nt -w:file -fork:8'
	  alias john-nt-korelogic='for ruleset in `grep KoreLogicRules /etc/john/john.conf | cut -d: -f 2 | cut -d\] -f 1`; do find /root/dic/ -name "*" | xargs -t -I dic john -fork:8 --rules:${ruleset} -w:dic --format:nt nt; done'
